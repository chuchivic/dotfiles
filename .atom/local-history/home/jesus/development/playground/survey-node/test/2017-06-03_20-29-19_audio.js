...
describe('audio',function(){
  describe('getAudioPath',function(){
    it('should save with correct name', async function(done){
      try{
        ...
        var audioUrl = await audio.getAudioPath(text);
        expect(audioUrl).to.equal()
        assert.equal(audioUrl,webhook_url + '/' + md5(text) + '.wav');
        done();
      }catch(err){
        done(err);
      }
